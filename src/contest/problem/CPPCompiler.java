...
public class CPPCompiler implements Compiler
{
	...
	public void compileProblemEntry(ProblemEntry problemEntry)
	{
        ...
        String sFilename = sFileDir + sProbName + ".cpp";
        String sExeName = sFileDir + sProbName;
		File exeFile = new File(sExeName);
		if (exeFile.exists())
		{
			exeFile.delete();
		}
		...
		try
		{
			...
			Process pCompile = r.exec("g++ " + sFilename + " -o " + sExeName);
			pCompile.waitFor();
